...
// Customer Emails.
add_action( 'woocommerce_email_before_order_table', array( $this, 'store_instructions' ), 10, 3 );
...
$this->form_fields = array(
	...
	'checkout_description' => array(
		...
		'default'     => 'Please select your mobile money payment method to send the requested total amount via an online mobile money platform (CashApp, Venmo, GooglePay, ApplePay, Western Union, Moneygram, etc) or via a local mobile money agent and fill these fields out below to confirm that you have sent the total requested amount',
		'desc_tip'    => true,
	),
	'store_instructions'    => array(
		'title'       => 'Instructions',
		'type'        => 'textarea',
		'description' => 'Store Instructions that will be added to the thank you page and emails.',
...
public function thankyou_page() {
	...
	if ( $this->store_instructions ) {
		...
		echo '<p>We are checking our systems to confirm that we received the <strong style="text-transform:uppercase;">' . $total . '</strong> sent by <strong style="text-transform:uppercase;">' .  
		esc_html( $CustomerMOMOName ) . '</strong> using the following phone number: <strong>'. esc_html( $CustomerMOMONo ) . '</strong> along with the following reference code <strong>' . 
		esc_html( $MOMORefNo ) . '</strong> sent using ' . esc_html( $MOMOApp ) . ' so we can proceed with the shipping and delivery options you chose.</p><br><br>';
		
		echo 'Thank you for doing business with us, ' . esc_html( $CustomerMOMOName ) . '!<br> You will be updated regarding your order details soon<br>';
		
		echo '<p>Here are some additional store instructions: </p><br>' , wp_kses_post( wpautop( wptexturize( $this->store_instructions ) ) . PHP_EOL );
 ...
 */
public function store_instructions( $order, $sent_to_admin, $plain_text = false ) {
...
public function validate_fields() {
	...
	if( empty($customerPaymentMode) ) {
		wc_add_notice(  'Payment Transfer Method ' . $customerPaymentMode . ' is empty', 'error' );
		$sum = $sum++;
	}

	if( strlen($customerMomoName) < 3 ) {
		wc_add_notice(  'Mobile Money Customer Name ' . $customerMomoName . ' is too short', 'error' );
		$sum = $sum++;
	}
...
public function process_payment( $order_id ) {
	...
	if( !is_wp_error($order) ) {
		...
		$order->update_status( apply_filters( 'woocommerce_momo_process_payment_order_status', 'on-hold', $order ), __( 'Checking for payment', 'woocommerce' ) );
		...
		$note = 'Your order request was received!'.'<br><br>'.
			'We are checking our systems to confirm that we received the <strong style="text-transform:uppercase;">' . $total . '</strong> sent by <strong style="text-transform:uppercase;">' .  esc_html( $CustomerMOMOName ) . '</strong> using the following phone number: <strong>'. esc_html( $CustomerMOMONo ) . '</strong> along with the following reference code <strong>' . esc_html( $MOMORefNo ) . '</strong> sent using ' . esc_html( $MOMOApp ) . ' so we can proceed with the shipping and delivery options you chose.'.'<br><br>'.
			...
			'Kindest Regards,<br>'. 'Store Assistant';
